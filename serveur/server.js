...
if (!process.env.DATABASE_URL) {
    console.error("DATABASE_URL n'est pas configuree.");
    process.exit(1);
}
...
async function initDatabase() {
    ...
    await pool.query(`
        INSERT INTO contest (id, registrations, prize_pool, status)
        VALUES (1, 0, 0, 'open')
...
async function getContest() {
    const result = await pool.query(`SELECT * FROM contest WHERE id = 1`);
    return result.rows[0];
}

async function getRanking() {
    const result = await pool.query(`
        SELECT id, name, score, games, registered, winnings
        FROM players
...
async function updateWinner() {
    ...
    await pool.query(`UPDATE players SET winnings = 0`);

    if (ranking.length === 0) {
        return { winner: null, winnerPrize: 0, platformShare: 0 };
    }

    const contest = await getContest();
    const winner = ranking[0];
    const winnerPrize = Math.floor(contest.prize_pool * WINNER_PERCENTAGE);
    const platformShare = contest.prize_pool - winnerPrize;

    await pool.query(
        `UPDATE players SET winnings = $1 WHERE id = $2`,
        [winnerPrize, winner.id]
    );

    return { winner: winner.name, winnerPrize, platformShare };
}
    ...
    try {
        ...
        if (existing.rows.length > 0) {
            ...
            if (player.registered) {
                ...
                return res.json({
                    success: true,
                    message: "Joueur deja inscrit.",
                    alreadyRegistered: true,
                    player: { id: player.id, name: player.name, registered: true },
                    contest: {
                        registrations: contest.registrations,
                        prizePool: contest.prize_pool,
            ...
            const updated = await pool.query(
                `UPDATE players SET registered = TRUE, registration_paid = $1 WHERE id = $2 RETURNING *`,
            ...
            await pool.query(`
                UPDATE contest
                SET registrations = registrations + 1, prize_pool = prize_pool + $1
                WHERE id = 1
            `, [REGISTRATION_FEE]);
            ...
            return res.json({
                ...
                player: { id: updatedPlayer.id, name: updatedPlayer.name, registered: true },
                contest: {
                    registrations: contest.registrations,
        ...
        const result = await pool.query(
            `INSERT INTO players (name, score, games, registered, registration_paid, winnings)
             VALUES ($1, 0, 0, TRUE, $2, 0) RETURNING *`,
        ...
        console.log(`${player.name} inscrit -> +${REGISTRATION_FEE} HTG`);
...
app.post("/api/players", async (req, res) => {
    try {
        ...
        if (result.rows.length === 0) {
            return res.status(404).json({ success: false, message: "Joueur non inscrit." });
        }
        ...
        if (!player.registered) {
            return res.status(403).json({
                success: false,
                message: "Le joueur doit etre inscrit au concours."
            });
        }

        const updated = await pool.query(
            `UPDATE players SET score = score + $1, games = games + 1 WHERE id = $2 RETURNING *`,
            [score, player.id]
        );
        const updatedPlayer = updated.rows[0];
        ...
        console.log(`${updatedPlayer.name} +${score} points -> ${updatedPlayer.score} points`);
...
app.get("/api/players", async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: "Impossible de recuperer les joueurs." });
    }
});
...
app.get("/api/ranking", async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: "Impossible de recuperer le classement." });
    }
});
...
app.get("/api/health", async (req, res) => {
    try {
        ...
        res.json({
            ...
            database: "connected",
            service: "Konkou",
            players: Number(result.rows[0].count),
            registrations: contest.registrations,
            prizePool: contest.prize_pool,
